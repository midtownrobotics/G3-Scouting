...
export async function getTeamRows(team: number, maxError?: number, fromMatch?: number): Promise<FormResponseData[]> {
    const formDatas = (await Promise.all(
        (await FormModel.getForms(true)).map(f => f.getResponseData(maxError, fromMatch))
    )).filter(formData => formData !== null);
    
    return formDatas.map(formData => ({...formData, responses: formData.responses.filter(d => d.team == team)}))
}

export async function getMatchRows(match: number, maxError?: number, fromMatch?: number): Promise<FormResponseData[]> {
    const formDatas = (await Promise.all(
        (await FormModel.getForms(true)).map(f => f.getResponseData(maxError, fromMatch))
    )).filter(formData => formData !== null);
